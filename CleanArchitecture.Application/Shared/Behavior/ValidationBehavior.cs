...
namespace CleanArchitecture.Application.Shared.Behavior
{
    public sealed class ValidationBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : IRequest<TResponse>
    {
        ...
        public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> nest, CancellationToken cancellationToken)
        {
            if(!_validators.Any()) return await nest();
            ...
            if(_validators.Any())
            {
                ...
                var validationResults = await Task.WhenAll(_validators.Select(y => y.ValidateAsync(context,cancellationToken)));
